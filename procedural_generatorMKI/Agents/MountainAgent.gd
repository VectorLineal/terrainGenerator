...
var width: int
var noise_prob: float
var variance: float
var restart_rate: int
var direction_status = 0

func _init(tokens: int, rate: int, max_h: float, min_h: float, slope_d: float, width: int, noise_p: float, variance: float, list: Array, sea_level: float, heightImage: Image, random_gen: RandomNumberGenerator).(tokens):
